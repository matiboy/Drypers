...
public class MultiTouchController<T> {
        ...
        public boolean onTouchEvent(MotionEvent event) {
                try {
                        ...
                        if (DEBUG)
                                Log.i(this.getClass().toString(), "Got here 1 - " + multiTouchSupported + " " + mMode + " " + handleSingleTouchEvents + " " + pointerCount);
                        if (mMode == MODE_NOTHING && !handleSingleTouchEvents && pointerCount == 1)
                                // Not handling initial single touch events, just pass them on
                                return false;
                        if (DEBUG)
                                Log.i(this.getClass().toString(), "Got here 2");
                        ...
                        for (int histIdx = 0; histIdx <= histLen; histIdx++) {
                                ...
                                if (!multiTouchSupported || pointerCount == 1) {
                                        ...
                                        if (DEBUG)
                                                Log.i(this.getClass().toString(), "Got here 3");
                                        ...
                                        if (DEBUG)
                                                Log.i(this.getClass().toString(), "Got here 4");
                                        int numPointers = Math.min(pointerCount, MAX_TOUCH_POINTS);
                                        if (DEBUG && pointerCount > MAX_TOUCH_POINTS)
                                                Log.i(this.getClass().toString(), "Got more pointers than MAX_TOUCH_POINTS");
        ...
        private void decodeTouchEvent(int pointerCount, float[] x, float[] y, float[] pressure, int[] pointerIds, int action, boolean down, long eventTime) {
                if (DEBUG)
                        Log.i(this.getClass().toString(), "Got here 5 - " + pointerCount + " " + action + " " + down);
        ...
        if (DEBUG)
            Log.i(this.getClass().toString(), "anchorAtThisPositionAndScale()");
        ...
        private void multiTouchController() {
                if (DEBUG)
                        Log.i(this.getClass().toString(), "Got here 6 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " " + mCurrPt.isDown() + mCurrPt.isMultiTouch());

                switch (mMode) {
                case MODE_NOTHING:
            if (DEBUG)
                Log.i(this.getClass().toString(), "MODE_NOTHING");
                ...
                case MODE_DRAG:
            if (DEBUG)
                Log.i(this.getClass().toString(), "MODE_DRAG");
                ...
                case MODE_PINCH:
            if (DEBUG)
                Log.i(this.getClass().toString(), "MODE_PINCH");
                ...
                if (DEBUG)
                        Log.i(this.getClass().toString(), "Got here 7 - " + mMode + " " + mCurrPt.getNumTouchPoints() + " " + mCurrPt.isDown() + mCurrPt.isMultiTouch());
        }
    ...
    public int getMode() {
        ...
        public static class PointInfo {
                ...
                private void set(int numPoints, float[] x, float[] y, float[] pressure, int[] pointerIds, int action, boolean isDown, long eventTime) {
                        if (DEBUG)
                                Log.i(this.getClass().toString(), "Got here 8 - " + +numPoints + " " + x[0] + " " + y[0] + " " + (numPoints > 1 ? x[1] : x[0]) + " "
